...
\item negation/NOT 
   \subitem A \term{boolean operation} that produces the complement of the nominated truth value, designated with the \term{symbol} $\neg$.
   
\item conjunction/AND
   \subitem  A two-place logical connective that has the value TRUE if both of its \term{operands} are TRUE, otherwise a value of FALSE, designated with the \term{symbol}  $\wedge$.

\item disjunction/OR $\vee$
   \subitem A two-place logical connective that has the value TRUE if at least one of its \term{operands} are TRUE, otherwise a value of FALSE, designated with the \term{symbol}  $\vee$
